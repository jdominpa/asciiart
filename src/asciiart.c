...
void downscale_img_res(unsigned char *pixels, size_t w, size_t h, size_t comp, float downscale_factor)
{
    size_t rescaled_w = w/downscale_factor;
    size_t rescaled_h = h/downscale_factor;
    unsigned char *aux = stbir_resize_uint8_srgb(pixels, w, h, w*comp,
...
Ascii_char grayscale_to_ascii(unsigned char gray_value)
{
    // 0..255 (grayscale value) -> 0..9 (Ascii_char index)
    return gray_value * (ASCII_CHAR_COUNT-1) / 255;
}

void convert_pixels_to_ascii(unsigned char *pixels, unsigned char *downscaled_pixels, size_t w, size_t h)
{
    for (size_t y = 0; y < h; y += ASCII_CHAR_SIZE) {
        for (size_t x = 0; x < w; x += ASCII_CHAR_SIZE) {
            ...
            uint8_t out_of_bounds = (x+ASCII_CHAR_SIZE < w && y+ASCII_CHAR_SIZE < h) ? 255 : 0;
            for (size_t y_offset = 0; y_offset < ASCII_CHAR_SIZE; y_offset++) {
                for (size_t x_offset = 0; x_offset < ASCII_CHAR_SIZE; x_offset++) {
                    pixels[i + x_offset + w*y_offset] = out_of_bounds*((ascii_char_pixel_map[ascii_char][y_offset] >> x_offset) & 1);
                }
            }
        }
    }
...
int main(int argc, char **argv)
{
    ...
    size_t width, height;
    unsigned char *pixels = stbi_load(input_path, (int *)&width, (int *)&height, NULL, 1);
    unsigned char downscaled_pixels[width*height];
